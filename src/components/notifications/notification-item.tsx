...
export function NotificationItem({
  ...
  return (
    <div
      className={cn(
        "group relative flex gap-2.5 border-s-4 transition-all duration-200 sm:gap-3",
        visuals.borderClass,
        !notification.is_read ? cn(visuals.bgClass, "bg-brand-50/20") : "bg-white hover:bg-surface-50/80",
        compact ? "border-s-2 px-3 py-2 sm:border-s-4 sm:py-2.5" : "px-3 py-3"
      )}
    >
      {selectable && (
        <input
      ...
      <div
        className={cn(
          "flex shrink-0 items-center justify-center rounded-lg text-lg sm:rounded-xl",
      ...
      <div className="min-w-0 flex-1">
        ...
        >
          <div className="flex items-start justify-between gap-2">
            ...
            {!notification.is_read && (
              <span
                className={cn(
                  "mt-1 shrink-0 rounded-full bg-brand-500 ring-2 ring-brand-100",
                  compact ? "h-2 w-2 sm:mt-1.5 sm:h-2.5 sm:w-2.5" : "mt-1.5 h-2.5 w-2.5"
                )}
          ...
          <p
            className={cn(
              "mt-0.5 text-surface-600",
              compact ? "line-clamp-1 text-[11px] sm:line-clamp-2 sm:text-xs" : "text-sm"
            )}
          >
            {notification.body}
          </p>
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5 sm:mt-2 sm:gap-2">
            ...
            <span
              className={cn(
                "hidden rounded-full px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide ring-1 ring-inset sm:inline-flex sm:px-2 sm:text-[10px]",
                visuals.badgeClass
              )}
            >
        ...
        {(onMarkRead || onDelete || actions) && (
          <div className="mt-1.5 flex gap-1 opacity-100 sm:mt-2 sm:opacity-0 sm:group-hover:opacity-100">
