"use client";

import { useState } from "react";
import { AnimatePresence, motion } from "framer-motion";
import { RotateCcw, Search, SlidersHorizontal, X } from "lucide-react";
import type {
  VerificationSort,
  VerificationTypeFilter,
  VerificationUiFilters,
} from "@/lib/admin/verifications-display";
import { Input } from "@/components/ui/input";
import type { VerificationRequestStatus } from "@/types/database";
import { useTranslation } from "@/i18n/locale-provider";

interface AdminVerificationsFilterToolbarProps {
  searchInput: string;
  onSearchInputChange: (value: string) => void;
  onSearchSubmit: () => void;
  statusFilter: VerificationRequestStatus | "all";
  onStatusChange: (status: VerificationRequestStatus | "all") => void;
  dateFilter: string;
  onDateChange: (date: string) => void;
  uiFilters: VerificationUiFilters;
  onUiFiltersChange: (filters: VerificationUiFilters) => void;
  onReset: () => void;
  isPending?: boolean;
}

function FilterSelect({
  label,
  value,
  onChange,
  options,
}: {
  label: string;
  value: string;
  onChange: (value: string) => void;
  options: { value: string; label: string }[];
}) {
  return (
    <label className="flex min-w-[8.5rem] flex-col gap-1">
      <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-surface-400">
        {label}
      </span>
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="h-9 rounded-lg border border-surface-200 bg-white px-2.5 text-xs font-medium text-surface-800 shadow-sm outline-none transition-colors focus:border-brand-300 focus:ring-2 focus:ring-brand-500/20"
      >
        {options.map((option) => (
          <option key={option.value} value={option.value}>
            {option.label}
          </option>
        ))}
      </select>
    </label>
  );
}

function FilterFields({
  statusFilter,
  onStatusChange,
  dateFilter,
  onDateChange,
  uiFilters,
  onUiFiltersChange,
  labels,
}: {
  statusFilter: VerificationRequestStatus | "all";
  onStatusChange: (status: VerificationRequestStatus | "all") => void;
  dateFilter: string;
  onDateChange: (date: string) => void;
  uiFilters: VerificationUiFilters;
  onUiFiltersChange: (filters: VerificationUiFilters) => void;
  labels: {
    status: string;
    type: string;
    date: string;
    sort: string;
    statusOptions: { value: string; label: string }[];
    typeOptions: { value: string; label: string }[];
    sortOptions: { value: string; label: string }[];
    all: string;
    today: string;
    custom: string;
  };
}) {
  const datePreset =
    dateFilter === "today" ? "today" : dateFilter ? "custom" : "all";

  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <FilterSelect
        label={labels.status}
        value={statusFilter}
        onChange={(value) => onStatusChange(value as VerificationRequestStatus | "all")}
        options={labels.statusOptions}
      />
      <FilterSelect
        label={labels.type}
        value={uiFilters.type}
        onChange={(value) =>
          onUiFiltersChange({ ...uiFilters, type: value as VerificationTypeFilter })
        }
        options={labels.typeOptions}
      />
      <div className="flex min-w-[8.5rem] flex-col gap-1">
        <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-surface-400">
          {labels.date}
        </span>
        <div className="flex gap-2">
          <select
            value={datePreset}
            onChange={(event) => {
              const value = event.target.value;
              if (value === "all") onDateChange("");
              else if (value === "today") onDateChange("today");
              else onDateChange(new Date().toISOString().slice(0, 10));
            }}
            className="h-9 min-w-0 flex-1 rounded-lg border border-surface-200 bg-white px-2.5 text-xs font-medium text-surface-800 shadow-sm outline-none focus:border-brand-300 focus:ring-2 focus:ring-brand-500/20"
          >
            <option value="all">{labels.all}</option>
            <option value="today">{labels.today}</option>
            <option value="custom">{labels.custom}</option>
          </select>
          {datePreset === "custom" && (
            <Input
              type="date"
              value={dateFilter === "today" ? "" : dateFilter}
              onChange={(event) => onDateChange(event.target.value)}
              className="h-9 w-[9.5rem] shrink-0 text-xs"
            />
          )}
        </div>
      </div>
      <FilterSelect
        label={labels.sort}
        value={uiFilters.sort}
        onChange={(value) =>
          onUiFiltersChange({ ...uiFilters, sort: value as VerificationSort })
        }
        options={labels.sortOptions}
      />
    </div>
  );
}

export function AdminVerificationsFilterToolbar({
  searchInput,
  onSearchInputChange,
  onSearchSubmit,
  statusFilter,
  onStatusChange,
  dateFilter,
  onDateChange,
  uiFilters,
  onUiFiltersChange,
  onReset,
  isPending,
}: AdminVerificationsFilterToolbarProps) {
  const { t } = useTranslation();
  const [mobileOpen, setMobileOpen] = useState(false);

  const labels = {
    status: t.admin.verifications.filters.status,
    type: t.admin.verifications.filters.type,
    date: t.admin.verifications.filters.date,
    sort: t.admin.verifications.filters.sort,
    reset: t.admin.verifications.filters.reset,
    openFilters: t.admin.verifications.filters.openFilters,
    apply: t.admin.verifications.filters.apply,
    search: t.admin.verifications.searchButton,
    all: t.admin.verifications.filters.all,
    today: t.admin.verifications.statToday,
    custom: t.admin.verifications.filters.custom,
    statusOptions: [
      { value: "all", label: t.admin.verifications.filterAllStatuses },
      { value: "pending", label: t.admin.verifications.statusPending },
      { value: "approved", label: t.admin.verifications.statusApproved },
      { value: "rejected", label: t.admin.verifications.statusRejected },
    ],
    typeOptions: [
      { value: "all", label: t.admin.verifications.filters.all },
      { value: "identity", label: t.admin.verifications.filters.typeIdentity },
      { value: "with_address", label: t.admin.verifications.filters.typeWithAddress },
    ],
    sortOptions: [
      { value: "newest", label: t.admin.verifications.filters.sortNewest },
      { value: "oldest", label: t.admin.verifications.filters.sortOldest },
    ],
  };

  return (
    <>
      <motion.div
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.32 }}
        className="space-y-4 rounded-2xl border border-surface-200/80 bg-white p-4 shadow-sm sm:p-5"
      >
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
          <div className="relative min-w-0 flex-1">
            <Search
              className="pointer-events-none absolute start-4 top-1/2 h-5 w-5 -translate-y-1/2 text-surface-400"
              aria-hidden
            />
            <Input
              value={searchInput}
              onChange={(event) => onSearchInputChange(event.target.value)}
              onKeyDown={(event) => {
                if (event.key === "Enter") onSearchSubmit();
              }}
              placeholder={t.admin.verifications.searchPlaceholder}
              className="h-11 rounded-xl border-surface-200 bg-surface-50/50 ps-12 text-sm shadow-inner"
            />
            {isPending && (
              <span className="absolute end-4 top-1/2 -translate-y-1/2 text-xs text-surface-400">
                …
              </span>
            )}
          </div>

          <button
            type="button"
            onClick={onSearchSubmit}
            className="inline-flex h-11 items-center justify-center rounded-xl bg-brand-600 px-5 text-sm font-semibold text-white transition-colors hover:bg-brand-700"
          >
            {labels.search}
          </button>

          <button
            type="button"
            onClick={onReset}
            className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-surface-200 bg-white px-4 text-sm font-semibold text-surface-700 transition-colors hover:bg-surface-50"
          >
            <RotateCcw className="h-4 w-4" />
            {labels.reset}
          </button>

          <button
            type="button"
            onClick={() => setMobileOpen(true)}
            className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-surface-200 bg-white px-4 text-sm font-semibold text-surface-700 lg:hidden"
          >
            <SlidersHorizontal className="h-4 w-4" />
            {labels.openFilters}
          </button>
        </div>

        <div className="hidden lg:block">
          <FilterFields
            statusFilter={statusFilter}
            onStatusChange={onStatusChange}
            dateFilter={dateFilter}
            onDateChange={onDateChange}
            uiFilters={uiFilters}
            onUiFiltersChange={onUiFiltersChange}
            labels={labels}
          />
        </div>
      </motion.div>

      <AnimatePresence>
        {mobileOpen && (
          <>
            <motion.button
              type="button"
              aria-label={t.admin.verifications.cancel}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-50 bg-black/40 lg:hidden"
              onClick={() => setMobileOpen(false)}
            />
            <motion.div
              initial={{ y: "100%" }}
              animate={{ y: 0 }}
              exit={{ y: "100%" }}
              transition={{ type: "spring", stiffness: 340, damping: 34 }}
              className="fixed inset-x-0 bottom-0 z-[60] max-h-[85vh] overflow-y-auto rounded-t-3xl border border-surface-200 bg-white p-5 shadow-2xl lg:hidden"
            >
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-bold text-surface-900">{labels.openFilters}</h3>
                <button
                  type="button"
                  onClick={() => setMobileOpen(false)}
                  className="flex h-10 w-10 items-center justify-center rounded-xl border border-surface-200"
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
              <FilterFields
                statusFilter={statusFilter}
                onStatusChange={onStatusChange}
                dateFilter={dateFilter}
                onDateChange={onDateChange}
                uiFilters={uiFilters}
                onUiFiltersChange={onUiFiltersChange}
                labels={labels}
              />
              <button
                type="button"
                onClick={() => setMobileOpen(false)}
                className="mt-5 w-full rounded-xl bg-brand-600 py-3 text-sm font-semibold text-white"
              >
                {labels.apply}
              </button>
            </motion.div>
          </>
        )}
      </AnimatePresence>
    </>
  );
}
