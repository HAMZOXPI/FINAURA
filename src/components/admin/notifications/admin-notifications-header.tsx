...
export function AdminNotificationsHeader({ onBroadcastClick }: AdminNotificationsHeaderProps) {
    ...
    >
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight text-surface-900 sm:text-3xl">
          ...
          {t.admin.notifications.centerTitle}
        </h1>
        <p className="mt-2 max-w-2xl text-surface-500">{t.admin.notifications.centerSubtitle}</p>
      </div>

      <div className="flex flex-col items-stretch gap-3 sm:items-end">
        <div className="flex flex-wrap items-center justify-end gap-2">
          <Button type="button" onClick={onBroadcastClick} className="gap-2">
            <Megaphone className="h-4 w-4" />
            {t.admin.notifications.broadcastAction}
          </Button>
          <Link
            href="/admin/settings"
            className="inline-flex h-11 items-center gap-2 rounded-xl border border-surface-200 bg-white px-4 text-sm font-semibold text-surface-700 transition-colors hover:bg-surface-50"
          >
            <Settings className="h-4 w-4" />
            {t.admin.notifications.settingsAction}
          </Link>
        </div>
        ...
        <span className="inline-flex items-center gap-2 self-end rounded-full border border-emerald-200/80 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700">
          ...
          {t.admin.notifications.serviceOnline}
        </span>
      </div>
    </motion.header>
  );
